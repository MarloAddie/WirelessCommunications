...
load('Z_w2_test.mat');
z_w2 = Z_w; % Frequency data calculated in Part 2 for the second hydrophone
[Hw2, noiseVar2] = channelest(z_w2, ofdm_map, OFDM_PILOT); % Task 1
Hw2 = Hw2(ofdm_map==2,:); % Locating the channel for data subcarriers

load('Z_w3_test.mat');
z_w3 = Z_w; % Frequency data calculated in Part 2 for the third hydrophone
[Hw3, noiseVar3] = channelest(z_w3, ofdm_map, OFDM_PILOT); % Task 1
Hw3 = Hw3(ofdm_map==2,:); % Locating the channel for data subcarriers

% Locating frequency data for data subcarriers:
...
for i = 1:21
    ...
    LRcount2 = 2;
    for j = 1:1420
        Hk = [Hw1(j,i);Hw2(j,i);Hw3(j,i)]; % Est. channel for the kth subcarrier of three hydrophones
        Zk = [z_w1(j,i);z_w2(j,i);z_w3(j,i)]; % Received freq domain for the kth subcarrier of three hydrophones
        Hknorm = sqrt(Hk'*Hk);
        qk(j,i) = (Hk'*Zk)/(Hknorm); % Projected signal of kth subcarrier of three hydrophones
