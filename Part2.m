...
W = 24; % # of OFDM symbols

[d_tilde,Y,LFM] = ZP_OFDM_Tx(L,M,W);



% ZP-OFDM Transmitter Design & Validation:

function[d_tilde,Y,LFM] = ZP_OFDM_Tx(L,M,W)
...
%     title('Original Symbols (s)')
    ...
    Trx = 7.763;  % Approximated from spectrogram
    Ttx = 8.2695; %  sec
    a_hat = Ttx/Trx - 1;
    ...
    b = xcorr(OFDM_data_pre_old, Y_PB_re_tilde);
    figure
    plot(b)
